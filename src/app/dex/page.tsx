...
// 리그 색상 (tailwind purge 우회 — 인라인 스타일 사용)
...
// players.json → 팀+연도 단위 그룹핑 → AVG OVR 계산 → 정렬
function buildTeamGroups(players: PlayerSeason[], year: number): TeamGroup[] {
  ...
  // AVG OVR 내림차순
  return [...map.values()].sort((a, b) => b.avgOvr - a.avgOvr)
}

function avgOvrColor(avg: number): string {
  if (avg >= 93) return '#e9c349'   // secondary (골드)
  if (avg >= 88) return '#f3f4f6'   // on-surface (흰색)
  return '#9ca3af'                  // outline (회색)
}
...
export default function DexPage() {
  ...
  return (
    <div className="min-h-screen text-on-surface font-body-main">
      ...
      {/* ── 데스크톱 헤더 (fixed) ── */}
      <header className="hidden md:flex fixed top-0 left-0 w-full z-50 items-center justify-between px-10 py-4 border-b border-outline-variant/30 bg-[#14141c]/95 backdrop-blur-md">
        <div className="flex items-center gap-8">
          <Link href="/" className="font-ovr-display text-ovr-display tracking-tighter text-on-surface hover:text-secondary transition-colors">
            GRANDSLAM
          </Link>
          <nav className="flex gap-6">
            ...
            {(['MARKET', 'HALL OF FAME'] as const).map(item => (
              <span key={item} className="font-heading-md text-heading-md text-on-surface-variant hover:text-secondary transition-colors uppercase cursor-default">
                {item}
              </span>
            ))}
          </nav>
        </div>
        {/* 헤더 검색 — filter bar와 동기화 */}
        <div className="relative">
          <input
            ...
            placeholder="Search..."
            className="bg-surface-container-high border-none rounded text-on-surface font-body-main text-sm pl-3 pr-9 py-1.5 focus:ring-1 focus:ring-outline/50 w-56 outline-none"
          />
      ...
      <header className="flex md:hidden fixed top-0 left-0 w-full z-50 items-center justify-between px-5 py-4 border-b border-outline-variant/30 bg-[#14141c]/95 backdrop-blur-md">
        ...
        <span className="font-heading-md text-[14px] text-secondary uppercase">COLLECTION</span>
      </header>
        ...
        <div className="mb-8 md:mb-12">
          ...
          {!loading && (
            <p className="font-label-caps text-label-caps text-outline/50 mt-2 uppercase">
              {filteredTeams.length} Teams · {filteredTeams.reduce((s, g) => s + g.players.length, 0)} Players
            </p>
          )}
        </div>
          ...
          <div className="space-y-12">
            ...
            {filteredTeams.map(group => (
              <section key={group.key} className="space-y-4">
                ...
                {/* 5명 카드 그리드 (포지션 순서 TOP/JGL/MID/ADC/SUP) */}
